-module(flash_socket_fsm_handler).
...
%%-------------------------------------------------------------------------
start_link(UserData) ->
    gen_fsm:start_link(?MODULE, [UserData], []).
...
init([_UserData]) ->
	...
	%% TODO: extract data and shove it in #state{}
...
'WAIT_FOR_DATA'({data, Data}, #state{socket=S} = State) ->
    error_logger:info_msg("State: 'WAIT_FOR_DATA'. Got data: ~p!\n", [Data]),
